...
for (int i = 1; i <= testData.getRowNumbers(); i++) {
   ...
   // TestObject loginButton = findTestObject('button_LOGIN/Login Button')
	...
	WebUI.navigateToUrl('http://13.228.241.164:8081/hrp2/self_leaveprojection_newentry.jsp?')
	

	String ss = """
	var element = document.evaluate('/html/body/div[4]/form/div/input', document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
	if (element) {
		element.click();
	}
	"""
	WebUI.executeJavaScript(ss, null)
